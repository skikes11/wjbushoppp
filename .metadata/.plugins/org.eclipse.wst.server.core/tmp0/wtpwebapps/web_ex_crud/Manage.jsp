<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
 <%@ taglib uri = "http://java.sun.com/jsp/jstl/core" prefix = "c" %>
  <%@ taglib prefix = "fmt" uri = "http://java.sun.com/jsp/jstl/fmt" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<script src="https://use.fontawesome.com/b32121c496.js"></script>
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
<title>Insert title here</title>
<style type="text/css">
.card{
 	width: 100%;
    height: 300px;
    object-fit: cover;  
}
.form-inline{
	width: 100%;
    height: 75px;
}
.container{
	text-align: center;
}
.row img {
    width: 100%;
    height: 157px;
    object-fit: cover;
}
.dropdown{
left: 1280px;
}
.row{
	margin-left: 130px;
	margin-right: 130px;
}

.card-body{
	text-align: center;

}
.col-md-3{
	width: 260px;

}
.btn-import {
	margin-left: 145px;
}

.type{
margin-left: 792px;
}
.type-sort-import p{
	margin-top:5px;
	margin-right: 5px;
}
</style>


</head>
<body>
<nav class="navbar navbar-expand-sm bg-light navbar-light">
  <form class="form-inline" method="get">
  <a class="navbar-brand" href="#">
    <img src="data:image/jpeg;base64," alt="logo" style="width:40px;">
  </a>
 <ul class="navbar-nav">
    <li class="nav-item ">
      <a class="nav-link" href="listwaifu" onclick="MovePage_manage()">Home</a>
    </li>
    <li class="nav-item">
      <a class="nav-link" href="manage" onclick="MovePage_manage()" >Manage</a>
    </li>
    <li class="nav-item">
      <a class="nav-link" href="#" onclick="MovePage_manage()">Contact</a>
    </li>
    <li>
    <input class="form-control mr-sm-2" type="text" placeholder="Search">
    <button class="btn btn-success" type="submit">Search</button>
    </li>
    </ul>
  </form>
</nav>
	<form action=listwaifu method="get">
	<div class="container">
  		<h2>List Waifu</h2>
  		<h4> This is the list of Waifuuu Animeee </h4>
  </div>
  	
  	    <table class="type-sort-import">
  <tr> 
  <td>
  <div class="btn-import">
  	<a class="btn btn-primary" href="FormWaifuImport">Import Waifuu</a>
  	</div>
  </td>
  <td> <p class="type"> Type:  </p> </td>
  
  <td>	
  <select id="typeOfWaifu" name="type"   onchange="getType(this)">
  <option value="all">All</option>
  <option value="loli">Loli</option>
  <option value="BB">BB</option>
  </select>
  </td>
  <td> <p> Sort By: </p> </td>
  <td> 
  <select id="sort" class="sort" onchange="getSort(this)">
  <option value="priceUp">Price: Going Up</option>
  <option value="priceDown">Price: Going Down</option>
  <option value="AgeUp">Age: Going Up</option>
  <option value="AgeDown">Age: Going Down</option>
  <option value="name">Name: A->Z</option>
  </select>
  </td>
 
  </tr>
  </table>
	<div class="row">
	<c:forEach items="${listWaifu}" var="x">
	<div class="col-md-3">
	<div class="card"  >
    <img class="rounded"  src="${x.image}" alt="Card image" >
    <div class="card-body">
      <h6 class="card-title">${x.name}  &nbsp;&nbsp;&nbsp;${x.age}</h6>
      <p class="bloc-left-price">
		 <fmt:setLocale value = "en_US"/>
		 <fmt:formatNumber value = "${x.price}" type = "currency"/>
 		</p>
      <a href="FormWaifuUpdate?id=<c:out value='${x.id}'/>" class="btn btn-success">Update</a>
      &nbsp;&nbsp;
       <a href="waifuDelete?id=<c:out value='${x.id}'/>" class="btn btn-danger">Delete</a>
    </div>
  </div>
  </div>
	</c:forEach>
	</div>
	</form>
</body>

<script type="text/javascript">

document.getElementById('typeOfWaifu').value=localStorage.getItem("typeOfW_manage");
document.getElementById('sort').value=localStorage.getItem("typeOfS_manage");
	var type = [];
	type[0]="all";
function getType(selectObject) {
	 type[0] = selectObject.value;
	 localStorage.setItem("typeOfW_manage", type[0]);
	  if(type[0]==="all"){
		  window.location.href='manage';
	  }else if(type[0]==="loli"){
		  window.location.href='listWaifu_TypeLoli_Manage';
	  }else if(type[0]==="BB"){
		  window.location.href='listWaifu_TypeBB_Manage';
	  }
	  console.log(type[0]);
	  
	} 
	
function getSort(selectObject) {
	 var sort = selectObject.value;
	 localStorage.setItem("typeOfS_manage", sort);
	 type[0] = localStorage.getItem("typeOfW");
	  if(type[0]==="all" && sort==="priceUp"){
		  window.location.href='sortByPriceUp_Manage';
	  }else if (type[0]==="all" && sort==="priceDown"){
		  window.location.href='listWaifu_PriceGoingDown_Manage';
	  }else if (type[0]==="all" && sort==="AgeUp"){
		  window.location.href='listWaifu_AgeGoingUp_Manage';
	  }else if (type[0]==="all" && sort==="AgeDown"){
		  window.location.href='listWaifu_AgeGoingDown_Manage';	  
	  }else if (type[0]==="all" && sort==="name"){
		  window.location.href='listWaifu_Name_A_Z_Manage';		  
	  } else if (type[0]==="loli" && sort==="priceUp"){
		  window.location.href='sortByPriceUp_TypeLoli_Manage';
	  }else if (type[0]==="loli" && sort==="AgeDown"){
		  window.location.href='listWaifu_AgeGoingDown_TypeLoli_Manage';
	  }else if (type[0]==="loli" && sort==="AgeUp"){
		  window.location.href='listWaifu_AgeGoingUp_TypeLoli_Manage';
	  }else if (type[0]==="loli" && sort==="name"){
		  window.location.href='listWaifu_Name_A_Z_TypeLoli_Manage';
	  }else if (type[0]==="loli" && sort==="priceDown"){
		  window.location.href='listWaifu_PriceGoingDown_TypeLoli_Manage';
	  }else if (type[0]==="BB" && sort==="priceDown"){
		  window.location.href='listWaifu_PriceGoingDown_TypeBB_Manage';
	  }else if (type[0]==="BB" && sort==="AgeDown"){
		  window.location.href='listWaifu_AgeGoingDown_TypeBB_Manage';
	  }else if (type[0]==="BB" && sort==="AgeUp"){
		  window.location.href='listWaifu_AgeGoingUp_TypeBB_Manage';
	  }else if (type[0]==="BB" && sort==="name"){
		  window.location.href='listWaifu_Name_A_Z_TypeBB_Manage';
	  }else if (type[0]==="BB" && sort==="priceUp"){
		  window.location.href='sortByPriceUp_TypeBB_Manage';
	} 	
}

function MovePage_manage() {
	localStorage.setItem("typeOfW_manage", "all");
	localStorage.setItem("typeOfS_manage", "priceUp");
}
	
	
</script>













</html>