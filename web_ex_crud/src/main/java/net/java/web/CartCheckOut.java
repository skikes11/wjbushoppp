...
public class CartCheckOut extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		date.toString();
		int priceTotalLong= 0;
		billDAO.insertCustomer(IdCustomer, customerName, customerPhone, customerCity, customerDistrict, customerDetailAdd);
		billDAO.insertBill(IdBill, date, priceTotalLong, IdCustomer);
		 for (Map.Entry<Integer, BillDetail> entry : map.entrySet()) {
			 billDAO.insertBillDetail(entry.getValue().getUnitPrice(), entry.getValue().getQuantity(), IdBill, entry.getKey());
			 priceTotalLong+=entry.getValue().getUnitPrice();
		 }
		 
		 billDAO.Update_Bill(IdBill, priceTotalLong);
			} catch (Exception e) {
